...
<div class="d-block justify-content-between flex-wrap flex-end-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h5 class="h5"> Data Staff Guru</h5>
        <div class="d-flex">
            <h6>Home /<b class="text-primary"> Data Guru</b></h6>
        </div>  
        <div class="input-group m-3">
            <form action="{{ route('guru.home') }}" method="GET" class="d-flex align-items-center">
                <input type="text" name="data" value="{{ request('data') }}" class="form-control" placeholder="Cari...">
                <button type="submit" class="btn btn-info">Cari</button>
                @if(request('data'))
                    <a href="{{ route('guru.home') }}" class="btn btn-danger">Hapus</a>
                @endif
            </form>
        </div>    
        <div class="d-flex justify-content-end">
            <a href="{{ route('guru.create') }}" class="btn btn-primary mb-3">Tambah Pengguna</a>
        </div>
